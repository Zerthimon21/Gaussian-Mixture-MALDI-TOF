...
IsotopicCountFFT = function (pep,cutoff=.9999){
	...
	##CRC (2011 92 ed) values for isotopic point distrobutions for each element
	hd = c(0.99985, 0.00015) ##1,2  
	cd = c(0.98900, 0.01100) ##12,13
	nd = c(0.99634, 0.00366) ##14,15
	od = c(0.99760, 0.00039, 0.00201) ##16,17,18
	pd = c(1.00000) ##31
	sud = c(0.95020, 0.00750, 0.04210, 0.00000, 0.00020) ##32,33,34,35,36  sd used by basic r function
	...
	if (scount>0){
		sud2 = append(sud,rep(0,256-length(sud)))
		sud2 = fft(sud2)^scount
		MoleculeFFT=MoleculeFFT*sud2
	}
...
MultiNormCalc=function(SpectraRange,peptide,mass,sigma,mzError=0){
	...
	for(i in 1:length(h)){
		peaks=peaks+dnorm(mz-Mass,((i-1)*1.008664916)+mzError,sigma)*h[i]  ##Note correction for sigma spread seen in MALDI-TOF data, rm *1.008664916 for equal sigma peaks
	}
...
##quandrant gradient assent method fo crossing search space implemented
...
PeptideGenerator=function(PeptideNumber,PeptideSize,MassFilterStart,MassFilter=2*1.008664916){
...
##Essentially we calculate a MLE for lambda from Aik, then get this expected value of lambda^k^hat
Exx=0
...
#Remove negative values, our assumption of the Gaussian basis of the peaks is violated if we have negative values  
...
if(abs(Delta-DHp)<Tolerance && abs(Delta-DHp)<Tolerance)break ##Tolerance, can be broken up into vector for ease of use
